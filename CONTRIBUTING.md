# Contributing to Paterm

Paterm is a solo-maintained project with a strong product direction. Contributions are welcome, but **alignment matters more than volume**.
...
- Paterm has one active maintainer ([@crynta](https://github.com/crynta)).
...
For the architecture and how to contribute safely, see [PATERM.md](PATERM.md) and the [docs/ index](docs/README.md).
...
Paterm positions itself as **lightweight, fast, production-grade**. Every PR is reviewed against:
...
- Security review for changes to shell spawn, file system access, IPC commands
...
The most common way a PR breaks Paterm is a **local fix with global blast radius**: the diff solves one reported case, reads fine, passes type-check and clippy, and silently breaks the same subsystem in every other case. Review alone does not catch these. A test does.
...
- **Workspace authorization**: which directories spawns may operate in. Both the allow and the deny side.
...
- Paterm is terminal-first, not an IDE clone. Focused capabilities such as LSP, formatting, and previews belong when they stay fast, lazy, and resource-bounded.
...
security(workspace): tighten path guard
```
...
src-tauri/                  Rust backend
  src/
    ...
    modules/
      ...
      shell/                Oneshot shell command (editor format-on-save)
