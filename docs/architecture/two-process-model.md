...
Paterm is two processes: the Rust backend (`src-tauri/`) and the webview frontend (`src/`).

- **Rust owns all OS access**: PTY, file system, shell spawn, workspace authorization.
...
5. If the command touches the file system or shell, it must go through the existing guards (`security.ts` deny-list, workspace authorization registry).
...
- `shell_run_command` - one-shot subshell exec (used by editor format-on-save)

### Workspace (`src-tauri/src/modules/workspace.rs`)

- `workspace_authorize` / `workspace_current_dir` - the spawn cwd authorization registry
...
- New commands must be registered in `lib.rs` and guarded at the boundary (workspace auth, deny-list).
