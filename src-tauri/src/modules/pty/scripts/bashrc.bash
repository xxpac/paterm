# paterm-shell-integration (bashrc)
#
...
if [ -z "$__PATERM_HOOKS_LOADED" ]; then
  ...
  _paterm_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_paterm_urlencode "$PWD")"
    # Re-inject prompt-end marker in case a framework rebuilt PS1 (p10k, starship).
    if [ -z "$__PATERM_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __PATERM_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_paterm_precmd:"*) ;;
    *) PROMPT_COMMAND="_paterm_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
