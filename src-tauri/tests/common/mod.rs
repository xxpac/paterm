...
use paterm_lib::modules::fs::to_canon;
use paterm_lib::modules::workspace::{WorkspaceEnv, WorkspaceRegistry};
...
impl GitRepoFixture {
    pub fn new() -> Self {
        ...
        run_git_in(&canonical, &["config", "user.email", "test@paterm.local"]);
        run_git_in(&canonical, &["config", "user.name", "Paterm Test"]);
