...
export function useFileTree(rootPath: string | null, options?: Options) {
  ...
  const fetchChildren = useCallback(async (path: string) => {
    ...
    try {
      const entries = await invoke<DirEntry[]>("fs_read_dir", {
        ...
        // Always populate the gitignored flag so ignored entries stay dimmed;
        // the `ignore` crate handles this independently of any git tooling.
        gitDecorations: true,
  ...
  useEffect(() => {
    ...
    // Re-list loaded directories when the hidden-files preference changes.
