...
import type { PatermLspClient } from "./client";
...
async function createSession(
  ...
  const [{ TauriLspTransport }, { PatermLspClient }] = await Promise.all([
    import("./transport"),
    import("./client"),
  ]);

  if (PatermLspClient.hostPid === null) {
    PatermLspClient.hostPid = await invoke<number>("lsp_host_pid").catch(
      () => null,
    );
  }
  ...
  const client = new PatermLspClient({
    transport,
    rootUri,
