...
describe("matchBinding", () => {
  ...
  it("matches zoom keys via the physical code on non-US layouts", () => {
    // Some layouts report a dead key / different char for Ctrl+= and Ctrl+-;
    // the binding must still match through e.code (Equal / Minus).
    expect(
      matchBinding(event({ key: "´", code: "Equal", ctrlKey: true }), {
        key: "=",
        ctrl: true,
      }),
    ).toBe(true);
    expect(
      matchBinding(event({ key: "?", code: "Minus", ctrlKey: true }), {
        key: "-",
        ctrl: true,
      }),
    ).toBe(true);
    ...
    expect(
      matchBinding(event({ key: "x", code: "KeyX", ctrlKey: true }), {
        key: "=",
        ctrl: true,
      }),
    ).toBe(false);
