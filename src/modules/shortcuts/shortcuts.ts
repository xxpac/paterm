...
export const SHORTCUTS: Shortcut[] = [
  ...
  },
  {
    ...
    // where it's handed through to the shell (e.g. the tmux prefix or a TUI's
    // own binding). Mod+Shift+B always toggles, including from inside a terminal.
    defaultBindings: [
      { [MOD_PROP]: true, key: "b" },
...
export function matchBinding(
  ...
  if (id === "tab.selectByIndex") {
    ...
    // Fall back to the physical key. Non-US layouts (and Alt/AltGr combos)
    // rewrite e.key — e.g. Ctrl+= / Ctrl+- report a different character — so a
    // binding on "=" / "-" only matches reliably via e.code (Equal / Minus).
