...
import { zoomIn, zoomOut } from "@/lib/useZoom";
...
// treats a dropped path as a real paste while a plain shell gets the literal text.
...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-paterm-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  host.setAttribute("data-paterm-slot", String(slots.length));
  ...
  // Copy-on-select: mirror the current selection to the clipboard as soon as
  // it settles, matching the classic X11 / iTerm behaviour. Gated on a pref so
  // it never clobbers the clipboard unless the user opted in.
  ...
  // Ctrl + mouse wheel zooms the terminal font (standard terminal gesture).
  // Capture phase so it beats xterm's own scroll handling, and it stays robust
  // when a webview/keyboard layout swallows the Ctrl +/- key events.
  host.addEventListener(
    "wheel",
    (e) => {
      if (!e.ctrlKey || slot.currentLeafId === null) return;
      e.preventDefault();
  ...
  if (!fast) {
    ...
    if (p.snapshot) {
      try {
        ...
        console.warn("[paterm] snapshot replay failed:", e);
      }
    }
    if (p.altScreen) {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[paterm] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function disposeSlot(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[paterm] slot dispose failed:", e);
  }
  slot.host.remove();
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[paterm-webgl] dispose failed:", e);
  }
  try {
    const r = (
