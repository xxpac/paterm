...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S paterm-bin";
    case "debian":
      return `sudo apt install ./Paterm_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Paterm-${version}-1.x86_64.rpm`;
  }
}
...
export function UpdaterDialog() {
  // Auto-update check is disabled: the release infrastructure under the new
  // name does not exist yet, so we must not prompt automatically.
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Paterm v${manual.version} is available`
                  : `Paterm v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Paterm to finish installing."
              : downloading
                ? progress !== null
